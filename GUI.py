...
"""
mat=board_generator.make_board()
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if (mouse_on_grid(mouse_position)):
                selected = ((mouse_position[0] - grid_position[0]) // cell_size,
                            (mouse_position[1] - grid_position[1]) // cell_size)
    ...
    screen.fill(YGREY)
    pygame.draw.rect(screen, WHITE, (grid_position[0], grid_position[1], 450, 450))
    ...
    if selected != None:
        ...
        (selected[0] * cell_size) + grid_position[0], (selected[1] * cell_size) + grid_position[1], cell_size,
        cell_size))
        ...
        for j in range(len(s.board[0])):
            if (s.board[i][j] != 0):
                if (ques_bool[i][j]):
                    ...
                    (j * cell_size) + grid_position[0], (i * cell_size) + grid_position[1], cell_size, cell_size))
                text = font.render(str(s.board[i][j]), True, BLACK)
                screen.blit(text, (grid_position[0] + (j * cell_size) + (cell_size // 3),
                                   grid_position[1] + (i * cell_size) + (cell_size // 4)))

    pygame.draw.rect(screen, BLACK, (grid_position[0], grid_position[1], WIDTH - 150, HIGHT - 150), 2)
    for x in range(1, 10):
        if (x % 3 == 0):
            pygame.draw.line(screen, BLACK, (grid_position[0] + (x * cell_size), grid_position[1]),
                             (grid_position[0] + (x * cell_size), grid_position[1] + 450), 2)
            pygame.draw.line(screen, BLACK, (grid_position[0], grid_position[1] + (x * cell_size)),
